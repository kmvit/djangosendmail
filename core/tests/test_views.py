...
from core.models import Mailing, Message, Client
...
class TestStat(APITestCase):
    ...
    def test_mailing(self):
        ...
        mail_create = {"date_start": now(), "date_end": now(), 'time_start': now().time(),
                       ...
                       "operator_code": '412'}
        response = self.client.post('http://127.0.0.1:8000/api/mail-list/', mail_create)
    ...
    def test_client(self):
        ...
        response = self.client.post('http://127.0.0.1:8000/api/client-list/', client_create)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_message(self):
        response = self.client.get('http://127.0.0.1:8000/api/message-list/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_stat(self):
        self.test_mailing()
        url = 'http://127.0.0.1:8000/api/mailing-list'
